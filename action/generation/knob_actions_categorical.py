...
class CategoricalKnobGenerator(ActionGenerator):
    ...
    def __init__(
        ...
        vartype, enumvals = get_type_with_values(name) # return 'enum', ['minimal', 'replica', 'logical']
...
if __name__ == "__main__":
    enable = CategoricalKnobGenerator(Connector(), 'enable_seqscan', ['off']) # local
    actions = list(enable)
    for a in actions:
        print(a)
